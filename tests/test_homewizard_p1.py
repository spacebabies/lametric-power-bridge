...
async def test_homewizard_connect_success(mocker):
    """Test successful HomeWizard P1 HTTP bootstrap"""
    # Create source
    source = HomeWizardP1Source(host="192.168.2.87")
    ...
    mocker.patch('sources.homewizard_p1.httpx.AsyncClient', return_value=mock_client)
...
async def test_homewizard_connect_no_host_exits(mocker):
    """Test that connect exits when HOMEWIZARD_P1_HOST is empty"""
    mock_exit = mocker.patch('sources.homewizard_p1.sys.exit')

    source = HomeWizardP1Source(host="")
    await source.connect()
...
async def test_homewizard_stream_yields_power_readings(mocker):
    ...
    source = HomeWizardP1Source(host="192.168.2.87", poll_interval=0.01)  # Fast polling for tests
...
async def test_homewizard_stream_handles_device_busy(mocker):
    ...
    source = HomeWizardP1Source(host="192.168.2.87", poll_interval=0.01)
    ...
    mock_sleep = mocker.patch('sources.homewizard_p1.asyncio.sleep')
...
async def test_homewizard_stream_missing_active_power_field(mocker):
    ...
    # Mock sleep
    mocker.patch('sources.homewizard_p1.asyncio.sleep')
